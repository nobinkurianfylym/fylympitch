...
export default async function PublicProfilePage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const [{ data: projects }, { data: credits }, { data: producerProfile }, { data: me }] = await Promise.all([
    supabase.from("projects").select("id, title, genre, format, logline, poster_path, slug, love_count, budget_currency, finance_secured_usd, funding_needed_usd")
                ...
                <Link key={p.id} href={`/projects/${p.slug ?? p.id}`}
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <div className="mt-4 pt-4 border-t border-line flex items-center justify-between text-[12px]">
                      ...
                      {(p as any).finance_secured_usd && <span className="text-emerald-700">Secured {formatBudget((p as any).finance_secured_usd, (p as any).budget_currency)}</span>}
                      {p.funding_needed_usd && <span className="text-gold">Seeking {formatBudget(p.funding_needed_usd, (p as any).budget_currency)}</span>}
                    </div>
                  </div>
                </Link>
              ))}
