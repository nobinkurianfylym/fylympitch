      ...
      <div className="w-full max-w-2xl">
        ...
        <form action={formAction} className="space-y-10">
          ...
          <section className="card p-6">
            <p className="text-[11px] tracking-[0.22em] uppercase text-ash mb-5">Your identity</p>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={email}
                  readOnly
                  className="field w-full bg-parchment text-ash cursor-not-allowed"
                />
              </div>
              <div>
                <label className="field-label mb-1 block">Company / Production house</label>
                <input
                  type="text"
                  value={company}
              ...
              {ROLE_TYPES.map((r) => (
                <button key={r.key} type="button" onClick={() => setRoleType(r.key)}
                  className={`text-left px-4 py-3 rounded-card border text-[13px] transition-all
                    ${roleType === r.key ? "border-gold bg-gold/10 text-ink font-medium" : "border-line bg-white text-ash hover:border-ink/30 hover:text-ink"}`}>
                  {r.label}
                </button>
              ))}
            </div>
          ...
          {/* 3. Genres */}
          <section>
            <p className="text-[11px] tracking-[0.22em] uppercase text-ash mb-4">What excites you? <span className="normal-case font-normal">(tick all)</span></p>
              ...
              {GENRES.map((g) => (
                <button key={g} type="button" onClick={() => setGenres(toggle(genres, g))}
                  className={`px-4 py-2 rounded-full text-[12px] tracking-[0.1em] border transition-all
                    ${genres.includes(g) ? "bg-ink text-ivory border-ink" : "bg-white border-line text-ash hover:border-ink/40 hover:text-ink"}`}>
                  {g}
                </button>
              ))}
            </div>
              ...
              {FORMATS.map((f) => (
                <button key={f} type="button" onClick={() => setFormats(toggle(formats, f))}
                  className={`px-5 py-2.5 rounded-full text-[13px] border transition-all
                    ${formats.includes(f) ? "bg-ink text-ivory border-ink" : "bg-white border-line text-ash hover:border-ink/40 hover:text-ink"}`}>
                  {f}
                </button>
              ))}
            </div>
