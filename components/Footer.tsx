import Link from "next/link";

function FylymLogo() {
  return (
    <svg width="110" height="110" viewBox="0 0 110 110" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="FYLYM">
      <defs>
        {/* Rainbow gradient for the bottom arc */}
        <linearGradient id="ft-rainbow" x1="0%" y1="0%" x2="100%" y2="0%" gradientUnits="userSpaceOnUse">
          <stop offset="0%"   stopColor="#FF3B30" />
          <stop offset="22%"  stopColor="#FF9500" />
          <stop offset="44%"  stopColor="#FFCC00" />
          <stop offset="62%"  stopColor="#34C759" />
          <stop offset="78%"  stopColor="#007AFF" />
          <stop offset="100%" stopColor="#AF52DE" />
        </linearGradient>
        {/* F bar gradient */}
        <linearGradient id="ft-fbar" x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%"   stopColor="#FF3B30" />
          <stop offset="33%"  stopColor="#FFCC00" />
          <stop offset="66%"  stopColor="#34C759" />
          <stop offset="100%" stopColor="#007AFF" />
        </linearGradient>
        {/* Glow for orbital dot */}
        <filter id="ft-glow" x="-80%" y="-80%" width="260%" height="260%">
          <feGaussianBlur stdDeviation="2.5" result="blur" />
          <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
        </filter>
        {/* Star glow */}
        <filter id="ft-star" x="-200%" y="-200%" width="500%" height="500%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge>
        </filter>
      </defs>

      {/* ── White arc — top ~300° ── */}
      {/* Arc from ~120° to ~60° clockwise (the long way) */}
      {/* cx=55 cy=50 r=44 */}
      {/* Start: 120° → x = 55 + 44·cos(120°) = 55−22 = 33, y = 50 + 44·sin(120°) = 50+38 = 88 */}
      {/* End:   60° → x = 55 + 44·cos(60°)  = 55+22 = 77, y = 50 + 44·sin(60°)  = 50+38 = 88 */}
      {/* The white arc goes from bottom-left to bottom-right the LONG way (300°) */}
      <path
        d="M 33 88 A 44 44 0 1 1 77 88"
        stroke="rgba(255,255,255,0.85)"
        strokeWidth="1.4"
        strokeLinecap="round"
        fill="none"
      />

      {/* ── Rainbow arc — bottom 60° ── */}
      {/* From bottom-left (120°) to bottom-right (60°) the SHORT way */}
      <path
        d="M 33 88 A 44 44 0 0 0 77 88"
        stroke="url(#ft-rainbow)"
        strokeWidth="1.8"
        strokeLinecap="round"
        fill="none"
      />

      {/* ── Orbital dot — at ~50° on the circle ── */}
      {/* x = 55 + 44·cos(−50°) ≈ 55+28.3 = 83.3, y = 50 + 44·sin(−50°) ≈ 50−33.7 = 16.3 */}
      {/* Star rays */}
      <g filter="url(#ft-star)" opacity="0.6">
        <line x1="83" y1="16" x2="83" y2="10" stroke="white" strokeWidth="0.8"/>
        <line x1="83" y1="16" x2="83" y2="22" stroke="white" strokeWidth="0.8"/>
        <line x1="83" y1="16" x2="77" y2="16" stroke="white" strokeWidth="0.8"/>
        <line x1="83" y1="16" x2="89" y2="16" stroke="white" strokeWidth="0.8"/>
        <line x1="83" y1="16" x2="78.8" y2="11.8" stroke="white" strokeWidth="0.5"/>
        <line x1="83" y1="16" x2="87.2" y2="20.2" stroke="white" strokeWidth="0.5"/>
        <line x1="83" y1="16" x2="87.2" y2="11.8" stroke="white" strokeWidth="0.5"/>
        <line x1="83" y1="16" x2="78.8" y2="20.2" stroke="white" strokeWidth="0.5"/>
      </g>
      {/* Dot ring */}
      <circle cx="83" cy="16" r="4.5" stroke="#8B5CF6" strokeWidth="1.2" fill="none" filter="url(#ft-glow)" opacity="0.9"/>
      {/* Dot center */}
      <circle cx="83" cy="16" r="2" fill="white"/>
    </svg>
  );
}

function FylymWordmark() {
  return (
    <div className="flex items-end gap-0 leading-none">
      {/* F with rainbow bar */}
      <span className="relative inline-block">
        <span
          className="font-sans text-[28px] font-[700] tracking-[-0.01em] text-ivory"
          style={{ letterSpacing: "-0.01em" }}
        >
          <span className="relative">
            F
            {/* Rainbow bar accent on the F crossbar */}
            <span
              className="absolute"
              style={{
                left: "2px",
                top: "9px",
                width: "12px",
                height: "2.5px",
                background: "linear-gradient(90deg, #FF3B30, #FFCC00, #34C759, #007AFF)",
                borderRadius: "1px",
                display: "block",
              }}
            />
          </span>
        </span>
      </span>
      <span
        className="font-sans text-[28px] font-[700] tracking-[-0.01em] text-ivory"
        style={{ letterSpacing: "-0.01em" }}
      >
        YLYM
      </span>
    </div>
  );
}

function SocialIcon({ href, label, children }: { href: string; label: string; children: React.ReactNode }) {
  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={label}
      className="text-ivory/40 hover:text-ivory transition-colors"
    >
      {children}
    </a>
  );
}

const COL_HEAD = "text-[10px] tracking-[0.2em] uppercase font-[600] text-ivory/40 mb-5 block relative";
const COL_LINE = "absolute bottom-0 left-0 w-6 h-[1.5px] mt-1";
const LI = "text-[13px] text-ivory/60 hover:text-gold transition-colors leading-none";

export function Footer() {
  return (
    <footer className="bg-deep text-ivory overflow-hidden relative">
      <div className="max-w-6xl mx-auto px-6 pt-16 pb-0 relative z-10">

        {/* ── Top row ── */}
        <div className="md:flex justify-between gap-12">

          {/* Left — logo + tagline + social */}
          <div className="max-w-[220px] shrink-0">
            <div className="flex items-center gap-3 mb-5">
              <FylymLogo />
            </div>
            <FylymWordmark />
            <p className="mt-4 text-[12px] leading-relaxed text-ivory/45 font-normal">
              Funding intelligence for filmmakers. Connecting creators with grants, funds, producers and investors worldwide.
            </p>
            {/* Social icons */}
            <div className="mt-6 flex items-center gap-5">
              <SocialIcon href="https://x.com/fylym" label="X / Twitter">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.742l7.73-8.835L1.254 2.25H8.08l4.259 5.63zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                </svg>
              </SocialIcon>
              <SocialIcon href="https://linkedin.com/company/fylym" label="LinkedIn">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                </svg>
              </SocialIcon>
              <SocialIcon href="https://instagram.com/fylym" label="Instagram">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/>
                </svg>
              </SocialIcon>
              <SocialIcon href="https://youtube.com/@fylym" label="YouTube">
                <svg width="20" height="18" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                </svg>
              </SocialIcon>
            </div>
          </div>

          {/* Right — 4 columns */}
          <div className="mt-10 md:mt-2 grid grid-cols-2 sm:grid-cols-4 gap-10 text-[13px] flex-1">

            {/* Opportunities */}
            <div>
              <span className={COL_HEAD}>
                Opportunities
                <span className={COL_LINE} style={{ background: "linear-gradient(90deg,#FF3B30,#FF9500)" }} />
              </span>
              <ul className="space-y-3.5">
                <li><Link href="/projects" className={LI}>Film Projects</Link></li>
                <li><Link href="/funds?type=development" className={LI}>Grants</Link></li>
                <li><Link href="/funds?type=development" className={LI}>Funds</Link></li>
                <li><Link href="/funds?type=development" className={LI}>Labs</Link></li>
                <li><Link href="/funds?type=packaging_markets" className={LI}>Markets</Link></li>
                <li><Link href="/funds?type=packaging_markets" className={LI}>Co-Productions</Link></li>
                <li><Link href="/funds?type=tax_incentives" className={LI}>Tax Incentives</Link></li>
                <li><Link href="/funds/submit" className={LI}>Submit an Opportunity</Link></li>
              </ul>
            </div>

            {/* Industry */}
            <div>
              <span className={COL_HEAD}>
                Industry
                <span className={COL_LINE} style={{ background: "linear-gradient(90deg,#FFCC00,#34C759)" }} />
              </span>
              <ul className="space-y-3.5">
                <li><Link href="/signup?role=producer" className={LI}>Producers</Link></li>
                <li><Link href="/signup?role=producer" className={LI}>Production Companies</Link></li>
                <li><Link href="/signup?role=producer" className={LI}>Studios</Link></li>
                <li><Link href="/signup?role=producer" className={LI}>Investors</Link></li>
                <li><Link href="/signup?role=producer" className={LI}>Sponsors</Link></li>
                <li><Link href="/signup?role=producer" className={LI}>Brand Integration</Link></li>
              </ul>
            </div>

            {/* Platform */}
            <div>
              <span className={COL_HEAD}>
                Platform
                <span className={COL_LINE} style={{ background: "linear-gradient(90deg,#007AFF,#5AC8FA)" }} />
              </span>
              <ul className="space-y-3.5">
                <li><a href="#features" className={LI}>How it Works</a></li>
                <li><a href="#features" className={LI}>FYLYMPITCH Engine™</a></li>
                <li><a href="#pricing" className={LI}>Pricing</a></li>
                <li><Link href="/signup" className={LI}>Join Free</Link></li>
                <li><Link href="/login" className={LI}>Sign In</Link></li>
              </ul>
            </div>

            {/* Legal */}
            <div>
              <span className={COL_HEAD}>
                Legal
                <span className={COL_LINE} style={{ background: "linear-gradient(90deg,#AF52DE,#FF2D55)" }} />
              </span>
              <ul className="space-y-3.5">
                <li><a href="/privacy" className={LI}>Privacy</a></li>
                <li><a href="/terms" className={LI}>Terms</a></li>
                <li><a href="/cookies" className={LI}>Cookies</a></li>
                <li><a href="mailto:nobinkurian@yahoo.com" className={LI}>Contact</a></li>
                <li><a href="mailto:nobinkurian@yahoo.com?subject=Report Issue" className={LI}>Report Issue</a></li>
              </ul>
            </div>
          </div>
        </div>

        {/* ── Copyright bar ── */}
        <div className="hairline-gold mt-14 pt-5 pb-6 flex items-center justify-between flex-wrap gap-3">
          <p className="text-[10px] tracking-[0.2em] uppercase text-ivory/30 font-normal">
            © {new Date().getFullYear()} PITCH.FYLYM · A FYLYM Company · Ernakulam, Kerala
          </p>
          <p className="text-[10px] tracking-[0.14em] uppercase text-ivory/20 font-normal">
            Intelligent matchmaking for independent film
          </p>
        </div>
      </div>

      {/* ── Ghost watermark ── */}
      <div
        className="absolute bottom-0 right-0 pointer-events-none select-none overflow-hidden"
        aria-hidden="true"
        style={{ lineHeight: 1 }}
      >
        <span
          className="font-sans font-[800] text-ivory/[0.04] block"
          style={{ fontSize: "clamp(80px, 14vw, 180px)", letterSpacing: "-0.04em" }}
        >
          PITCH.FYLYM
        </span>
      </div>
    </footer>
  );
}
